...
/* EXTERNS */
/*--------------------------------------------------------------------------*/
extern Scheduler *external_scheduler;
...
static void thread_shutdown()
{
    ...
    Console::puts(" is shutting down\n");

    external_scheduler->terminate(current);

    // This should never happen
    Console::puts("ERROR: thread_shutdown returned from scheduler->terminate()\n");
    assert(false);
}
